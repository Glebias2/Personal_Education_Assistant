...
def create_course(request_form: NewCourse):
    ...
    course_repository.create(request_form)
    return {"success": True}
...
def list_courses():
    ...
    return [{"id": cid, "title": title, "teacher_id": teacher_id} for (cid, title, teacher_id) in rows]
...
def delete_course_file(course_id: int, file_record_id: int):
    ...
    # Получаем file_id из SQL перед удалением
    rows = course_repository.get_course_files(course_id)
...
def get_course_info(course_id: int):
    ...
    info_list = []
    for part in info:
        id, title, teacher_id, exam_questions, storage_id = part
        info_list.append({"id": id, "title": title, "teacher_id": teacher_id, "exam_questions": exam_questions, "storage_id": storage_id})
...
def delete_course(course_id: int):
    ...
    # Удаляем векторное хранилище Weaviate, если оно есть
    storage_id = course_repository.get_storage_id(course_id)
    if storage_id:
        try:
            StorageManager.delete_storage(storage_id)
        except ValueError:
            pass  # хранилище уже удалено или не существует
